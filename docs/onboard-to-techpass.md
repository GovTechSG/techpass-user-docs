...
```mermaid
graph LR
A("You have an official business email address that is not a government email<br>address and does not resemble a personal email address<br>For example#58;<br>bob#64;accenture.com<br>alice#64;gmail.com") -->|Yes|B(Onboard to TechPass as vendors](onboard-vendors-to-techpass))
    B --> C{Decision}
    C -->|One| D[Result one]
    C -->|Two| E[Result two]
```
