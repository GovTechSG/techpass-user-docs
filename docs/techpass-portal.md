# TechPass portal

Users can access the [TechPass portal](https://portal.techpass.gov.sg) only from a non-SE GSIB devices.  This portal allows SGTS tenants to set up SSO for their SGTS products and manage IAM for their users.

 Refer to [TechPass Tenant Guide](https://docs.developer.tech.gov.sg/docs/techpass-tenant-guide/#/) to learn more about setting up SSO and manage users.

?> You need to log in with your TechPass account to view this user guide.

