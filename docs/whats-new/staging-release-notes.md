...
| **Enhancement** | Removed `userType` from Automation API | We have removed the `userType` field from the Automation API payloads. Previously, `userType` was used to denote whether an account was a Guest or Member in Azure AD, causing inaccuracies as 
