...
| **Enhancement** | Webhook domain whitelist update in staging environment. | The staging environment now accepts webhooks originating from *wogaa* and *ngrok* domains. Webhooks sent from these domains will be processed and handled within the staging environment. |
...
| **Fix** | Restriction on numbers in first and last names. | Previously, users were able to update their first and last names with numbers. It is now disabled. |
| **Fix** | Fix for Previously, when a significant volume of concurrent API calls was initiated, there was a possibility of receiving inaccurate responses. This inconsistency was caused by a racing condition that occurred during high concurrency scenarios.  We have fixed this issue. | 
