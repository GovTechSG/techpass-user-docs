...
| **Change** | Allow tenants to send emails from `<tenant namespace>@console.developer.gov.sg` | Tenants can now send email broadcasts from the `<tenant namespace>@<dev console domain>` email address. This sender email address is  automatically whitelisted.
